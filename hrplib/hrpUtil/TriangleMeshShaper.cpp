...
#if 0
void triangulate(const MFVec3f& vertices, const vector<int>& polygon0, vector<int>& out_triangles)
{
    vector<int> polygon(polygon0);

    Vector3 ccs(0.0);
    int size = polygon.size();
    for(int i=0; i < size; ++i){
        MFVec3f& a = vertices[polygon[i]];
        MFVec3f& b = vertices[polygon[(i+1)%size]];
        ccs[0] += a[1]*b[2] - a[2]*b[1];
        ccs[1] += a[2]*b[0] - a[0]*b[2];
        ccs[2] += a[0]*b[1] - a[1]*b[0];
    }
    //ccs /= norm2(ccs)
    ...
    while(true) {
        ...
        int target = -1;
        for(int i=0; i < size; ++i){
            Convexity = calcConvexity(vertices, polygon, ccs, i);
            if(Convexity == FLAT){
                target = i;
                break;
                ...
                if(isEar()){
                    target = i;
                    out_triangles.push_back(polygon[(i - 1) % size]);
                    out_triangles.push_back(polygon[i]);
                    out_triangles.push_back(polygon[(i + 1) % size]);
                    break;
                }
            }
        ...
        int j = target;
        for(int i = ear + 1, i < size; ++i){
            polygon[j++] = polygon[i];
        }
        polygon.pop_back();
    }

    out_triangles.push_back(polygon[0]);
    out_triangles.push_back(polygon[1]);
    out_triangles.push_back(polygon[2]);
}


Convexity calcConvexity
(const MFVec3f& vertices, const vector<int>& polygon, const Vector3& ccs, int ear)
{
    Vector3 ear_ccs(0.0);
    int size = polygon.size();
    MFVec3f& a0 = vertices[polygon[(ear-1)%size]];
    MFVec3f& b0 = vertices[polygon[ear]];
    MFVec3f& c0 = vertices[polygon[(ear+1)%size]];
    ...
    if(fabs(ear_ccs) < 1.0e-10) return FLAT;

    return (dot(ear_ccs, ccs) > 0.0) ? CONVEX : CONCAVE;
}
...
bool TriangleMeshShaper::isEar
(const MFVec3f& vertices, const vector<int>& polygon, const Vector3& ccs, int ear)
{

    LineType(origin, direction);
