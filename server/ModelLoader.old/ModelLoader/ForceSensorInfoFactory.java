package jp.go.aist.hrp.simulator;

import javax.vecmath.*;
// VRML97
import com.sun.j3d.loaders.vrml97.impl.*;

/**
 * ForceSensorInfoFactory class
 * ForceSensorÍÑ¡£
 * @author K Saito (Kernel Co.,Ltd.)
 * @version 1.0 (2002/02/25)
 */
public class ForceSensorInfoFactory
    extends SensorInfoFactory
{
    public void setParam(SensorInfo_impl info,String defName, VrmlSceneEx scene)
    {
        info.maxValue_ = new double[6];
        
        double[] def = {-1,-1,-1};
        double[] temp;
        
        temp = ProtoFieldGettor.getVectorValue(defName, scene, "maxForce",def );
        
        info.maxValue_[0] = (double)temp[0];
        info.maxValue_[1] = (double)temp[1];
        info.maxValue_[2] = (double)temp[2];
        
        temp = ProtoFieldGettor.getVectorValue(defName, scene, "maxTorque",def );
        
        info.maxValue_[3] = (double)temp[0];
        info.maxValue_[4] = (double)temp[1];
        info.maxValue_[5] = (double)temp[2];
        
    }
    public SensorType getType(){
        return SensorType.FORCE_SENSOR;
    }
}
