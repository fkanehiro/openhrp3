...
import com.generalrobotix.ui.util.GrxServerManager.GrxServer;
    ...
    private void updateBtn(int localDim){
        ...
        if( GrxServerManager.vecButton.elementAt(localDim).getText().equals(START))
        {
            ...
            if( pi.id.equals("NameService")) {
				...
				{
					com = new String[] {"/bin/sh" , "-c" , "rm "+ GrxServerManager.homePath_ + ".OpenHRP-3.1/omninames-log/*"};
				}
				else{
					com = new String[] {"cmd" , "/c" , "del " + "\"" + GrxServerManager.WIN_TMP_DIR  + "omninames-*.*" + "\""};
				}
				try{
