...
public class GrxProcessManager {
	...
	private JTextAreaEx outputArea_;
	private StringBuffer outputBuffer_;
	private JMenu menu_ = null;
	private JPopupMenu popup = new JPopupMenu();
	private JMenuItem menuItemClearAll_ = new JMenuItem("Clear All");

	public GrxProcessManager() {
		this_ = this;
	}

	public GrxProcessManager(java.util.List pInfo) {
		this_ = this;
		for (int i = 0; i < pInfo.size(); i++)
			process_.add(new AProcess((ProcessInfo) pInfo.get(i)));
	}

	public GrxProcessManager(ProcessInfo[] pInfo) {
		this_ = this;
		for (int i = 0; i < pInfo.length; i++)
			process_.add(new AProcess(pInfo[i]));
	}

	public static GrxProcessManager getInstance() {
		if (this_ == null) {
	...
	public void updateIO() {
		for (int i = 0; i < size(); i++) {
			...
			if (p.showOutput_) {
				...
				outputArea_.setCaretPosition(outputArea_.getText().length());
			}
		}
		try {
			Thread.sleep(10);
	...
	public JMenu getMenu() {
		if (menu_ == null) {
			menu_ = new JMenu("ProcessManager");
			menu_.addMenuListener(new javax.swing.event.MenuListener() {
				public void menuSelected(javax.swing.event.MenuEvent e) {
					menu_.removeAll();
					for (int i = 0; i < size(); i++) {
						// processManager_.loadDefaultConfig();
						final AProcess p = get(i);
						final JCheckBoxMenuItem jcbm = new JCheckBoxMenuItem(p.pi_.id, p.isRunning());
						menu_.add(jcbm);
	...
	public JPanel getOutputPanel() {
		if (outputPanel_ == null) {
			outputPanel_ = new JPanel(new java.awt.BorderLayout());
			outputPanel_.add(new JScrollPane(getOutputArea()),
	...
	public JTextArea getOutputArea() {
		if (outputArea_ == null) {
			outputArea_ = new JTextAreaEx();
			outputArea_.setEditable(false);
			outputBuffer_ = new StringBuffer();

			menuItemClearAll_.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					outputArea_.setText("");
					outputBuffer_ = new StringBuffer();
				}
			});
			outputArea_.addMouseListener(new java.awt.event.MouseAdapter() {
	...
	private void _updatePopup() {
		popup.removeAll();
		for (int i = 0; i < size(); i++) {
			final AProcess p = get(i);
			JCheckBoxMenuItem item = new JCheckBoxMenuItem(p.pi_.id, p.showOutput_);
			item.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					p.showOutput_ = ((JCheckBoxMenuItem) e.getSource()).getState();
