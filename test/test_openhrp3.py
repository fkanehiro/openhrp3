...
class TestCompile(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        f.write(code)
        f.close()
        super(TestCompile, self).__init__(*args, **kwargs)

    def pkg_config_variable(self, var):
        return subprocess.check_output("%s pkg-config openhrp3.1 --variable=%s"%(self.PKG_CONFIG_PATH, var), shell=True, text=True).rstrip()
    ...
    def check_if_file_exists_from_rospack(self, fname):
        pkg_dname = subprocess.check_output(['rospack','find','openhrp3']).rstrip()
        pkg_path = os.path.join(pkg_dname, fname)
        pkg_ret = os.path.exists(pkg_path)
    ...
    def test_compile_pkg_config(self):
        ...
        print("`"+cmd+"` =",subprocess.check_output(cmd, shell=True, text=True, stderr=STDOUT))
    ...
    def _test_compile_move_ankle(self):
        ...
        print("`"+cmd1+"` =",subprocess.check_output(cmd1, shell=True, text=True, stderr=STDOUT))
        print("`"+cmd2+"` =",subprocess.check_output(cmd2, shell=True, text=True, stderr=STDOUT))
    ...
    def test_idl_dir(self):
        ...
        print("`"+cmd+"`"+fname+" = "+os.path.join(subprocess.check_output(cmd, shell=True, text=True).rstrip(), fname))
        self.assertTrue(os.path.exists(os.path.join(subprocess.check_output(cmd, shell=True, text=True).rstrip(), fname)))
    ...
    def test_sample_samplerobot(self):
        ...
        # check if model file exists
        print("`"+cmd+"`"+fname+" = "+os.path.join(subprocess.check_output(cmd, shell=True, text=True).rstrip(), fname))
        self.assertTrue(os.path.exists(os.path.join(subprocess.check_output(cmd, shell=True, text=True).rstrip(), fname)), "cmd = %r, fname = %r"%(cmd, fname))
        #
