...
public class SceneGraphModifier {
     ...
     * @version  1.1 Thu Apr 11 2002    Geometry複製処理を追加
     */
    public void modifyRobot(GrxModelItem robot)
...
/* for GrxUI
        for (
            Enumeration children = robot.bgRoot_.getAllChildren();
            children.hasMoreElements();
        ) {
            Node node = (Node)children.nextElement();
            robot.traverse(new BoundingBoxCreator());
        }
...
*/
        ...
        // 全体を囲む
        for (int i = 0; i < robot.bgRoot_.numChildren(); i++) {
            _calcUpperLower(robot.bgRoot_.getChild(i), new Transform3D());
        }
     ...
     * @version  1.1 Thu Apr 11 2002    Geometry複製処理を追加
     */
    public void modifyEnvironment(Manipulatable environment) {
        init_ = true;
        mode_ = CREATE_BOUNDS;

        TransformGroup tg = environment.getTransformGroupRoot();
        _cloneGeometry(tg); // Geometry複製処理
        _setCapabilities(tg);
