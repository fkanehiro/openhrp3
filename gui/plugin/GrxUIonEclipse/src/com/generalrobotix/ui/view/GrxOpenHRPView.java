...
public class GrxOpenHRPView extends GrxBaseView {
	...
	private DynamicsSimulator currentDynamics_ = null;
...
//	private DynamicsSimulator getDynamicsSimulator(boolean update) {
	...
	private boolean initController() {
		...
		for (int i=0; i<models.size(); i++) {
			...
			if (_setupController(model, false) < 0) 
				return false;
			
			if (simParamPane_.isViewSimulate() && 
				_setupController(model, true) < 0) 
				return false;
		}
		return true;
	}
	...
	private short _setupController(GrxModelItem model, boolean isViewSim) {
		...
		if (isViewSim) {
			controllerName = model.getProperty("imageProcessor");
			step = model.getDbl("imageProcessTime", 0.033);
		} else {
			controllerName = model.getProperty("controller");
			step = model.getDbl("controlTime", 0.005);
		}
		...
		AProcess p = pManager.processManager.get(controllerName);
		if (p != null) {
			org.omg.CORBA.Object o = GrxCorbaUtil.getReference(controllerName, nsHost_, nsPort_);
			try {
				o._non_existent();
				if (isInteractive_) {
                    boolean ans = MessageDialog.openQuestion(getParent().getShell(), "Restart the Controller", "Controller '"+controllerName+"' may already exist.\n" + "Restart it ?");
					if (ans == true) {
						p.stop();
						p.start(optionAdd);
					}
		...
		for (int j=0; ; j++) {
			...
			try {
				o._non_existent();
				ControllerFactory cfactory = ControllerFactoryHelper.narrow(o);
				Controller controller = cfactory.create(model.getName());
				controller.setDynamicsSimulator(currentDynamics_);
				
				if (isViewSim) {
					o = GrxCorbaUtil.getReference("ViewSimulator", nsHost_, nsPort_);
					ViewSimulator viewsim = ViewSimulatorHelper.narrow(o);
					controller.setViewSimulator(viewsim);
				}
				
				ControllerAttribute attr = new ControllerAttribute(model.getName(), controller, step);	
				attr.doCount_ = 0;
				attr.doFlag_  = false;
				controllers_.add(attr);
