	...
	implements ThreeDDrawable 
{
	...
	public Grx3DView(String name, GrxPluginManager manager, GrxBaseViewPart vp, Composite parent) 
	{
        ...
        //Frame frame_ = SWT_AWT.new_Frame(new Composite(parent,SWT.EMBEDDED));
		...
		btnPlayer_.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) {
				...
				Display.getDefault().syncExec( new Runnable(){
					public void run(){
						Shell mplayer = new Shell( getParent().getShell() );
						//mplayer.setSize(100, 100);
						mplayer.setText("Movie Player");
						mplayer.open();
					}
				});
				*/
    ...
    private void rec(){
		...
		stateLogger_.playLogTime(Math.max(manager_.getDelay(), step));//.play();
    }
