...
VrmlNodePtr VRMLParser::readNode(VrmlNodeCategory nodeCategory)
{
  ...
  if(symbol){
    ...
    if(symbol==D_USE){
      scanner->readString();
      TDefNodeMap::iterator p = defNodeMap.find(scanner->stringValue);
      if(p != defNodeMap.end()){
	return p->second;
      } else {
	scanner->throwException("Unknown node");
      }
    }
  ...
  scanner->readCharEx('{', "Syntax error 0");

  if(symbol){
    node = readSpecificNode(nodeCategory, symbol);
  } else {
    node = readProtoInstanceNode(scanner->stringValue, nodeCategory);
  }

  scanner->readCharEx('}', "Syntax error 1");
...
VrmlNodePtr VRMLParser::readSpecificNode(VrmlNodeCategory nodeCategory, int symbol)
{
  VrmlNodePtr node;
...
// ######
  ...
  default: scanner->throwException("unknown node");

  }

  if(!node->isCategoryOf(nodeCategory)){
    scanner->throwException("illegal node type");
  }
...
//==================================================================================================
/*!
    @brief      read DirectionalLight node

    @note       <BR>

    @date       2008-03-06 K.FUKUDA <BR>

    @return     read DirectionalLightPtr
*/
//==================================================================================================
