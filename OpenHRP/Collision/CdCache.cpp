...
CdModelSet::CdModelSet()
{
	...
	pointsCount_ = 0;
	trisCountAlloced_   = 3;
	pointsCountAlloced_ = trisCountAlloced_ * 3; // ここを変更するとaddTriangle()で落ちる可能性があります。
	points_ = new IceMaths::Point[pointsCountAlloced_];
...
{
	//OPCODEのモデルに三角形を追加する。
	//Opcode::MeshInterface に載せる,頂点と三角形のインデックスを追加。
	if(pointsCount_ == pointsCountAlloced_){
		udword n = pointsCountAlloced_*2;
		if(n == 0) n = 3;
		IceMaths::Point *p = new IceMaths::Point[n];
