...
import jp.go.aist.hrp.simulator.IoControlPlugin;
import jp.go.aist.hrp.simulator.IoControlPluginHelper;
import jp.go.aist.hrp.simulator.LongSequenceHolder;
import jp.go.aist.hrp.simulator.Plugin;
import jp.go.aist.hrp.simulator.PluginHelper;
import jp.go.aist.hrp.simulator.PluginManager;
import jp.go.aist.hrp.simulator.PluginManagerHelper;
import jp.go.aist.hrp.simulator.RobotStateHolder;
import jp.go.aist.hrp.simulator.SensorState;
import jp.go.aist.hrp.simulator.WorldStateHolder;
import jp.go.aist.hrp.simulator.stateProvider;
import jp.go.aist.hrp.simulator.stateProviderHelper;
...
import jp.go.aist.hrp.simulator.IoControlPluginPackage.StatusSeqHolder;
import jp.go.aist.hrp.simulator.IoControlPluginPackage.SwitchStatus;
...
public class GrxIOBClientView extends GrxBaseView {
	...
	public void control(List<GrxBaseItem> itemList) {
		...
		if (state_ == CONNECTED && time > interval_) {
			...
			wsx.setJointState(robotType_, actualStateH_.value.servoState);
			wsx.setCalibState(robotType_, calibStateH_.value);
			wsx.setPowerState(robotType_, actualStateH_.value.voltage, actualStateH_.value.current);
	...
	private void servoOn() {
		...
		if(ioCtrl_ != null) {
			...
			pluginManager_.sendMsg(":wait 1.0");
		}
		...
		if (ans == JOptionPane.OK_OPTION) {
			try {
				...
				if(ioCtrl_ != null){
					...
					pluginManager_.sendMsg(":wait 5.0");
				}
