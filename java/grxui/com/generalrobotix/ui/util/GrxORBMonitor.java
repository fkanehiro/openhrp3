...
public class GrxORBMonitor extends JPanel 
{
    ...
    public GrxORBMonitor(String strHost, String strPort) {
        ...
        if ( strHost.equals("localhost") )
        {
            cmbHost_ = new JComboBox(new String[]{"localhost"});
        }else{
            cmbHost_ = new JComboBox(new String[]{strHost,"localhost"});
        }
        cmbHost_ = new JComboBox(new String[]{strHost,"localhost"});
        cmbHost_.setEditable(true);
        cmbHost_.setPreferredSize(new Dimension(160, 20));
        ...
        fldPort_ = new JTextField(strPort);
