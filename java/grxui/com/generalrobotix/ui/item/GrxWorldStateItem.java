...
public class GrxWorldStateItem extends GrxTimeSeriesItem {
    ...
    public static final double DEFAULT_STEP_TIME = 0.001;
	...
	public GrxWorldStateItem(String name, GrxPluginManager manager) {
    	...
    	if ( System.getProperty("os.name").equals("Linux") ||
	    	...
	    	String localTmpDir = System.getenv("TMPFS_DIR").trim();
	    	if( localTmpDir.length() == 0 ){
	    		tempDirBase_ =  System.getenv("GRXUI_TEMP")+File.separator;
	    	} else {
	    		tempDirBase_ =  localTmpDir + File.separator; 
	    	}
    	} else {
    		tempDirBase_ =  System.getenv("GRXUI_TEMP")+File.separator;
    	}
	...
	public void addValue(Double t, Object obj) {
		...
		if (obj instanceof WorldStateEx) {
			...
			} catch (LogFileOutputException e){
                GrxDebugUtil.printErr("",e);
			}
	...
	private void _initLog() {
		...
		try {
			logger_.openAsWrite(stime, getProperty("method"));
			logger_.openAsRead();
	...
	public void extendTime(double time) {
		...
		double val = getDbl("logTimeStep",GrxWorldStateItem.DEFAULT_STEP_TIME);
		stime.setTimeStep(val);
		setDbl("logTimeStep",val);
		
		val = getDbl("totalTime", GrxWorldStateItem.DEFAULT_TOTAL_TIME) + time;
	    stime.setTotalTime(val);
	    setDbl("totalTime", val);
